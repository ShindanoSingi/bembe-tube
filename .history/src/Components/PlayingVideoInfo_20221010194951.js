...
function PlayingVideoInfo({videoObject, viewsFunc, getaddToPlaylist}) {
     ...
     return(videoObject.type === "video" ? (
          <div class="flex text-white gap-2 p-1 mx-auto">
               <div class="flex rounded-lg  items-center">
                    ...
                    <div class="p-1 px-2 flex flex-col justify-start">
                         <h5 class="text-xs videoTitle mb-2 md:text-lg w-screen">
                              {
                              videoObject.title.charAt(0) + videoObject.title.substring(1, 40) + "..."
                         }</h5>
                         <ul className="flex text-xs md:text-24 justify-evenly gap-4 text-[#AAAAAA]">
                              <li>{
