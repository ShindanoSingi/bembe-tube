...
const VideoCards = ({ videos, viewsFunc }) => {
  ...
  return (
    <div className="flex mt-1 overflow-auto md:flex-wrap md:h-auto max-h-80">
      {videos.map(
        (video, index) =>
          video.type === "video" && (
            <Link
            ...
            >
              <div
                key={index}
                class="flex gap-1 h-fit flex-row mb-2 items-center bg-white rounded-lg border shadow-md md:flex-row hover:bg-gray-100 md:w-1/2 dark:border-gray-700 dark:bg-gray-800 dark:hover:bg-gray-700"
              >
                <div className="">
                  <img
