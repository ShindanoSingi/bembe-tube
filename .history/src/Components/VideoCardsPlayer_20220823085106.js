...
const VideoCards = ({ videos, viewsFunc }) => {
  ...
  return (
    <div className="flex flex-wrap mt-1 cards-container">
      {videos.map(
        (video, index) =>
          video.type === "video" && (
            <Link
            ...
            >
              <div
                key={index}
                class="flex gap-1 h-fit flex-row mb-2 items-center bg-white rounded-lg border shadow-md md:flex-row hover:bg-gray-100 md:max-w-screen-1/2 dark:border-gray-700 dark:bg-gray-800 dark:hover:bg-gray-700"
              >
                <div className="">
                  <img
