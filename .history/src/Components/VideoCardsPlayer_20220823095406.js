...
const VideoCards = ({ videos, viewsFunc }) => {
  ...
  return (
    <div className="flex flex-wrap overflow-auto gap-2.5 mt-1 justify-content-center cards-container">
      {videos.map(
        (video, index) =>
          video.type === "video" && (
            <Link
            ...
            >
              <div
                key={index}
                class="card flex gap-1 h-fit flex-row mb-1 items-center bg-white rounded-lg border shadow-md md:flex-row hover:bg-gray-100 md:w-84 dark:border-gray-700 dark:bg-gray-800 dark:hover:bg-gray-700"
              >
                <div className="">
                  <img
