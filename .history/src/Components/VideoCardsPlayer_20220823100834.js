...
const VideoCards = ({ videos, viewsFunc }) => {
  ...
  return (
    <div className="flex flex-wrap gap-2 mt-1 overflow-auto justify-content-center lg:flex-col cards-container">
      {videos.map(
        (video, index) =>
          video.type === "video" && (
            <Link
            ...
            >
              <div
                key={index}
                class="card flex gap-1 h-fit flex-row mb-1.5 items-center bg-white rounded-lg border shadow-md md:flex-row lg:flex-col hover:bg-gray-100 md:w-84 dark:border-gray-700 dark:bg-gray-800 dark:hover:bg-gray-700"
              >
                <div className="">
                  <img
