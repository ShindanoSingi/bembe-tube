...
const VideoCards = ({ videos, viewsFunc }) => {
  ...
  return (
    <div className="flex flex-wrap gap-2 overflow-auto lg:px-0.5 lg:overflow-auto lg:col justify-content-center cards-container">
      {videos.map(
        (video, index) =>
          video.type === "video" && (
            <Link
            ...
            >
              <div
                key={index}
                class="card flex gap-1 h-fit flex-row mb-1.5 lg:mb-0.5 items-center bg-white rounded-lg border shadow-md md:flex-row hover:bg-gray-100 w-screen md:w-84 lg:rounded-r-lg dark:border-gray-700 dark:bg-gray-800 dark:hover:bg-gray-700"
              >
                <div className="">
                  <img
                ...
                <div className="flex gap-2 mt-4 flex-column">
                  <p className="ml-0.5 lg:w-60 text-sm tracking-tight text-gray-900">
