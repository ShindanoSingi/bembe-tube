...
const VideoCards = ({ videos, viewsFunc }) => {
  ...
  return (
    <div className="relative flex flex-wrap mt-1 overflow-auto botton-10 md:mt-1 lg:overflow-auto lg:flex-row justify-content cards-container">
      {videos.map(
        (video, index) =>
          video.type === "video" && (
            <Link
            ...
            >
              <div
                key={index}
                class="card flex gap-1 h-fit flex-row mb-1.5 lg:mb-0.5 items-center bg-white rounded-lg border shadow-md md:flex-row hover:bg-gray-100 lg:rounded-r-lg dark:border-gray-700 dark:bg-gray-800 dark:hover:bg-gray-700"
              >
                <div className="">
                  <img
