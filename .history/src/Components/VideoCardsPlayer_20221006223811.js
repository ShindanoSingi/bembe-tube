...
const VideoCards = ({videos, viewsFunc}) => {
     ...
     return !videos ? <Wait/>: (
          <div className="flex flex-wrap top-[21rem] max-h-[10rem] fixed px-1 mx-auto overflow-auto lg:left-72 bg-[black] min-h-full  md:flex md:fle-row md:flex-wrap md:gap-1 lg:flex md:mt-8 lg:overflow-auto lg:flex-row md:flex-row justify-content ">
               {
               videos.map((video, index) => video.type === "video" && (
                    <Link key={
                         ...
                         <div key={index}
                              class="card w-screen h-32 flex gap-1 lg:mb-1.5 flex-row mb-1.5 items-center bg-white rounded-lg border shadow-md md:flex-row hover:bg-gray-900 lg:rounded-r-lg">

                              <div className="">
                                   <img className=" min-w-[7rem] h-[10rem]  md:h-auto object-cover md:w-48 rounded-l-lg md:rounded-none md:rounded-l-lg"
                                        src={
                                             video.thumbnail.url
                                        }
                                        alt=""
