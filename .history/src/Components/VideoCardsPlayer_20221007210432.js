...
const VideoCards = ({videos, viewsFunc}) => {
     ...
     return !videos ? <Wait/>: (
          <div className="flex fixed flex-wrap no-scrollbar gap-2 top-[21rem] max-h-[10rem] mb-16  mx-auto overflow-scroll lg:left-72 bg-[#1E1E1E] min-h-full  md:flex md:fle-row md:flex-wrap md:gap-1 lg:flex md:mt-8 lg:overflow-auto lg:flex-row md:flex-row justify-content ">
               {
               videos.map((video, index) => video.type === "video" && (
                    <Link key={
                         ...
                         <div class="flex justify-center  max-w-2xl mx-1 border border-gray-700 rounded-lg"
                              key={index}>
                              <div class="flex h-[8rem] w-[98vw] gap-2 md:flex-row md:max-w-xl rounded-lg bg-[#1E1E1E] ">
