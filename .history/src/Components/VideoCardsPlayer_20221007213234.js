...
const VideoCards = ({videos, viewsFunc}) => {
     ...
     return !videos ? <Wait/>: (
          <div className="flex fixed flex-wrap no-scrollbar gap-2 top-[21rem] max-h-[53.5vh] overflow-scroll lg:left-72 bg-[#1E1E1E]  md:flex md:fle-row md:flex-wrap md:gap-1 lg:flex md:mt-8 lg:overflow-auto lg:flex-row md:flex-row justify-content ">
               {
               videos.map((video, index) => video.type === "video" && (
                    <Link key={
                         ...
                         <div class="flex justify-center items-center  max-w-2xl border border-gray-700 rounded-lg"
                              key={index}>
