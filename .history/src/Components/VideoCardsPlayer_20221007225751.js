...
const VideoCards = ({videos, viewsFunc}) => {
     ...
     return videos.length < 1 ? <Wait/>: (
          <div className="flex fixed md:grid md:grid-cols-2 flex-wrap no-scrollbar gap-2 top-[21rem] max-h-[53.5vh] mb-2  mx-auto overflow-scroll lg:left-72 bg-[#1E1E1E]  md:flex md:fle-row md:flex-wrap md:gap-1 lg:flex md:mt-8 lg:overflow-auto lg:flex-row md:flex-row justify-content ">
               {
               videos.map((video, index) => video.type === "video" && (
                    <Link key={
                         ...
                         <div class="flex justify-center   max-w-2xl md:w-[30rem] mx-1 border border-stone-900 rounded-lg"
                              key={index}>
