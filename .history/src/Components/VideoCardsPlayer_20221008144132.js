...
const VideoCards = ({videos, viewsFunc}) => {
     ...
     return videos.length < 1 ? <Wait/>: (
          <div className="cards-container flex fixed flex-wrap no-scrollbar gap-2  top-[20.5rem] md:fixed  max-h-[42rem] mb-2 md:flex overflow-scroll lg:left-72  md:items-center  md:flex-wrap md:gap-1 lg:flex  lg:overflow-wrap lg:flex-row  md:flex-row justify-content ">
               {
               videos.map((video, index) => video.type === "video" && (
                    <Link key={
                         ...
                         <div class="flex justify-center md:grid card  mx-1 rounded-lg"
                              key={index}>
                              <div class="flex h-[8rem] w-[96.5vw] md:w-[97.8vw] lg:max-w-md  gap-2 md:flex-row  rounded-lg bg-[#1E1E1E] ">
