...
const VideoCards = ({videos, viewsFunc}) => {
     ...
     return videos.length < 1 ? <Wait/>: (
          <div className="cards-container flex fixed flex-wrap no-scrollbar gap-2  top-[20.5rem] md:fixed md:mx-auto  max-h-[42rem] mb-2  overflow-scroll lg:left-72    md:flex-wrap  lg:flex  lg:overflow-auto lg:flex-row   justify-content ">
               {
               videos.map((video, index) => video.type === "video" && (
                    <Link key={
                         ...
                         <div class="grid border border-neutral-600  w-[98vw] md:w-[48.5vw] rounded-lg"
                              key={index}>
                              <div class="flex h-[8rem] w-[96.5vw] single-card  gap-2 md:flex-row mx-auto rounded-lg bg-[#1E1E1E] ">
