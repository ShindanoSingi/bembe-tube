...
const VideoCards = ({videos, viewsFunc}) => {
     ...
     return videos.length < 1 ? <Wait/>: (
          <div className="cards-container flex fixed flex-wrap no-scrollbar gap-2   top-[20.5rem] md:fixed md:mx-auto  max-h-[42rem] mb-2  overflow-scroll   md:flex-wrap  lg:flex xl:flex xl:flex-col  lg:overflow-auto lg:flex-row   justify-content ">
               {
               videos.map((video, index) => video.type === "video" && (
                    <Link key={
                         ...
                         <div class="grid border border-neutral-600  w-[98vw] md:w-[48.5vw] xl:w-[27rem] mx-1  rounded-lg"
                              key={index}>
