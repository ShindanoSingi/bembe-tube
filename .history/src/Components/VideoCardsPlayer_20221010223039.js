...
const VideoCards = ({videos, viewsFunc}) => {
     ...
     return videos.length < 1 ? <Wait/>: (
          <div className="cards-container flex fixed flex-wrap no-scrollbar gap-2 xl:top-[0rem]  xl:fixed xl:left-[85rem]  top-[20.5rem] md:fixed md:mx-auto  max-h-[42rem] mb-2  overflow-scroll   md:flex-wrap  lg:flex xl:flex xl:flex-col  lg:overflow-auto lg:flex-row   justify-content ">
               {
               videos.map((video, index) => video.type === "video" && (
                    <Link key={
                         ...
                         <div class="grid w-[98vw] md:w-[48.5vw] xl:w-[30rem] mx-1  rounded-lg md:border-none"
                              key={index}>
                              <div class="flex h-[8rem] w-[96.5vw] single-card  gap-2 md:flex-row rounded-none  rounded-lg bg-[#1E1E1E] ">
                                   <img className="w-[6rem] videoThumbnail h-[8rem] md:h-auto object-cover md:w-48 rounded-l-lg xl:rounded-none md:rounded-l-lg"
                                        src={
                                             video.thumbnail.url
                                        }
                                        alt=""
