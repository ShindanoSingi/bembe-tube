      ...
      {videos.map(
        (video, index) =>
          video.type === "video" && (
            ...
            >
              <div
                class="md:w-[48.5vw] mb-1 xl:w-[30rem] mx-1 gap-2  rounded-lg md:border-none"
                key={index}
              >
                <div class="flex h-[8rem] single-card  gap-2 md:flex-row  rounded-lg lg:rounded-none bg-[#1E1E1E]">
                  ...
                  <div className="flex flex-col gap-2 py-2">
                    <p className=" ml-0.5 md:invisible visible display-none px-2 w-[60vw] lg:w-60 text-sm text-white">
                      {video.title.charAt(0).toUpperCase() +
                        video.title.substring(1, 48).toLowerCase() +
                        "..."}{" "}
                    </p>
